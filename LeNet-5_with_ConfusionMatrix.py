...
for true_label in range(num_classes):
    ...
    for images, labels in test_loader:
        ...
        for i in range(len(labels)):
            if labels[i].item() == true_label and predicted[i].item() != true_label:
                confidence = probabilities[i][predicted[i]].item()
                if confidence > max_confidence:
                    max_confidence = confidence
                    most_confused_image = images[i].cpu().numpy()
                    misclassified_as = predicted[i].item()

    if most_confused_image is not None:
        most_confusing[true_label] = (misclassified_as, most_confused_image)


for true_label, (misclassified_as, image) in most_confusing.items():
    plt.figure()
    plt.imshow(image.squeeze(), cmap='gray')
    plt.title(f'True Label: {true_label}, Misclassified as: {misclassified_as}')
    plt.show()
